...
from kvform.models import KVField, KVModelName, KVRecord, KVValue
...
class KVModelMetaclass(type):
	...
	def __refresh_fields__(new_class, model_name):
		if model_name is not None:
			...
			try:
				new_class.model = KVModelName.objects.filter(code = model_name)
				if len(new_class.model) > 0:
					new_class.model = new_class.model[0]

					# retrieve all fields for the specified model/form
					fields = KVField.objects.filter(kv_model__code = model_name)
					for f in fields:
...
class KVModel(metaclass = KVModelMetaclass):
	...
	def __new__(cls, *args, **kwargs):
		obj = super(KVModel, cls).__new__(cls, *args, **kwargs)
		cls.__refresh_fields__(obj, "PROFILE")
		return obj
	...
	def save(self, commit = True):
		...
		if not self.unique_key:
			...
			fields = KVField.objects.filter(kv_model__code = self.model.code)
			for f in fields:
